class StoreCreditMinimumValidator < ActiveModel::Validator
  def validate(record)
    if record.apply_credit && Spree::Config[:use_store_credit_minimum] and record.item_total < Spree::Config[:use_store_credit_minimum].to_f and record.store_credit_amount > 0
